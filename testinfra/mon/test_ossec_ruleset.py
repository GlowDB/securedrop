...
email_suppressed_regex = re.compile(r"no_email_alert")
...
@pytest.mark.parametrize('log_event',
                         sdvars.log_events_with_ossec_alerts)
def test_ossec_emails_supressed_as_expected(Command, Sudo, log_event):
    with Sudo():
        c = Command('echo "{}" | /var/ossec/bin/ossec-logtest'.format(
        log_event["alert"]))
        assert "Alert to be generated" in c.stderr
        ...
        rule_id = rule_id_regex.findall(c.stderr)[0]
        assert rule_id == log_event["rule_id"]
        email_suppressed = email_suppressed_regex.findall(c.stderr)[0]
        assert email_suppressed == log_event["options"]
